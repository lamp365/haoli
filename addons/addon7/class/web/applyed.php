...
if($op == 'xinyuan'){
...
}else if($op == 'all_recorder'){
    //所有记录
    $condition = '';
    if(!empty($_GP['status'])){
        $condition .= " where status={$_GP['status']}";
    }
    $pindex = max(1, intval($_GP['page']));
    $psize = 50;
    $limit = ' limit '.($pindex-1)*$psize.','.$psize;
    ...
    if(!empty($awardlist)){
        foreach($awardlist as &$draw){
            ...
            //获取礼品
            $goods = mysqld_select("select title,award_type from ".table('addon7_award')." where id={$draw['award_id']}");
            $draw['g_title']    = $goods['title'];
            $draw['award_type'] = $goods['award_type'];
        }
        $total     = mysqld_selectcolumn('SELECT COUNT(*) FROM ' . table('addon7_change') . " {$condition}");
        $pager      = pagination($total, $pindex, $psize);
    }
    include   addons_page('all_change');

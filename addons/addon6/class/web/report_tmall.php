		...
		switch ( $_GP['template'] ){
			...
			case 2:
				...
				foreach($list as $item){
				    ...
				    foreach($item['goods'] as $itemgoods){
							$itemdline = $itemdline+1;
							$goods_gold = 0; // 统计包裹金额，控制在300以内，如果大于300 则标记颜色
							$index++;
							$num = $itemgoods['total'];
							...
							if ($itemgoods['type']==1 or !empty($item['lists'])){
							   // 如果组合中没有数据则忽视
							   if ( empty($itemgoods['lists']) ){
                                     message('组合商品:'.$itemgoods['title'].'，有未标记单品的产品，订单导出失败！','','error');
							   }
							   $itemgoods['lists'] = str_replace('，',',',$itemgoods['lists']);
							   $goods = explode(',',$itemgoods['lists']);
							   foreach ( $goods as $goods_value ){
								      $goods_value = trim(html_entity_decode($goods_value),chr(0xc2).chr(0xa0));
								       list($good_sn,$goods_num) = explode('*', $goods_value );
									   if ( empty($good_sn) or !is_numeric($goods_num) ){
                                            message($itemgoods['title'].'单品货号出错','','error');
									   }
									   $good_sn = trim(html_entity_decode($good_sn),chr(0xc2).chr(0xa0));
									   // 开始根据货号进行商品的查询；
									   $itemgoods = mysqld_select("SELECT * FROM ".table('tmall_dish')." WHERE dishsn = '".$good_sn."'");
									   if (!empty($itemgoods['brand'])) {
											$brand =  mysqld_select('SELECT brand FROM '.table('shop_brand').' WHERE id = '.$itemgoods['brand']);
											$itemgoods['brand'] = $brand['brand'];
									   }else{
									   ...
									   $itemgoods['total'] = $num * $goods_num;
                                       $goods_gold += $itemgoods['total'] * $itemgoods['productprice'];
									   // $itemgoods 在组合里还没取值
									   $itemgoods['num'] = $nums;
											$objPHPExcel->setActiveSheetIndex(0)		
												...
												->setCellValue('K'.$i,  $item['address_area']?$item['address_area']:'其它区')
												->setCellValue('L'.$i, '')
												...
												->setCellValue('AE'.$i, $itemgoods['productprice']);
									   $i++;
							   }
						       }else{
								            $goods_gold += $itemgoods['total'] * $itemgoods['productprice'];
					  ...
					  if ($goods_gold > 300){
							  for ( $icolor = $ii; $icolor < $i; $icolor ++ ){
								   $objPHPExcel->getActiveSheet()->getStyle('A'.$icolor.':AE'.$icolor)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
								   $objPHPExcel->getActiveSheet()->getStyle('A'.$icolor.':AE'.$icolor)->getFill()->getStartColor()->setARGB('FFFF00');
								   $styleArray = array(  
										  'borders' => array(  
												'allborders' => array(  
									 ...
									 $objPHPExcel->getActiveSheet()->getStyle('A'.$icolor.':AE'.$icolor)->applyFromArray($styleArray);
							  }
					   }
				}	
				$objPHPExcel->getActiveSheet()->getStyle('D')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
							   ...
							   foreach ( $goods as $goods_value ){
								      if($itemdline!=0){
									   ...
									   if ( empty($goods_id )  or  !is_numeric($goods_num ) ){
									    ...
									    $itemgoods = mysqld_select('SELECT * FROM '.table('tmall_dish').' WHERE dishsn = '.$goods_id);
										// 开始获取品牌
										$brand =  mysqld_select('SELECT brand FROM '.table('shop_brand').' WHERE id = '.$itemgoods['brand']);
									    $objPHPExcel->setActiveSheetIndex(0)	
													...
													->setCellValue('S'.$i, $itemgoods['productprice'])

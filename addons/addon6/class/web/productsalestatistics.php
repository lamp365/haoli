...
$sta = mysqld_select("SELECT id,department FROM ".table('tmall_staff')." WHERE admin='".$admin."'");

if (empty($sta) OR empty($sta['department'])) {
  message('抱歉，非店铺人员无法查看！');
}

$tmall_id = array();
$tmall_id['tma_id'] = $sta['department'];
$tmall_id['sta_id'] = $sta['id'];
...
if ($operation == 'into') {
 ...
 message('导入完成！',refresh(),'success');
  ...
  // 订单页
  $where2 = "deleted<>1";
  if (!empty($order_number) OR !empty($order_tag) OR !empty($begintime) OR !empty($endtime)) {
    $where2 = "id<>0";
  }
  ...
  $order = mysqld_selectall("SELECT SQL_CALC_FOUND_ROWS * FROM ".table('tmall_order')." WHERE ".$where2." ORDER BY createtime DESC");
  // 品牌库
  ...
  $data_value = trim(html_entity_decode($_GP['ajax_value']),chr(0xc2).chr(0xa0));
  ...
  if (!empty($ad_title)) {
    ...
    message('产品名称不能为空！',refresh(),'error');
  }
  ...
  $list = mysqld_selectall("SELECT * FROM ".table('tmall_order')." WHERE ".$where." ORDER BY address_address ASC,createtime DESC");
  foreach ($list as &$lv) {
    ...
    mysqld_update('tmall_order',array('deleted'=>1),array('id'=>$lv['id']));
  }
  unset($lv);
  // dump($list);
  // return;

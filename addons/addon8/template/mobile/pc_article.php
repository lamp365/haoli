...
<body>
	...
	<div class="viewport article">
		<div class="viewport" style="width:1190px;">
			<div class="re title"><img src="<?php echo WEBSITE_ROOT . 'themes/' . $theme . '/__RESOURCE__'; ?>/recouse/images/prompt_icon.png">温馨提示： 觅海不会以订单异常、系统升级为由，要求您提供银行卡号、密码等信息，或让您支付额外费用，请谨防钓鱼链接和诈骗电话！ </div>
			<div class="re" style="width:100%;border:none;">
				<div class="article-left">
					<ul>
						<?php if(!empty($artile_tree)){ ?>
						<?php foreach($artile_tree as $row){  ?>
						<li>
							<span class="article-name"><?php echo $row['name'];?></span><span class="right-icon"></span>
							<?php if(!empty($row['son'])){ $index=0;?>
							<div class="child-div">
								<?php foreach($row['son'] as $id => $son){  $index++ ;?>
								<div class="<?php if($index==1) echo 'first-div';?>">
									<a href="<?php echo create_url('mobile',array('name'=>'addon8','do'=>'article','id'=>$id));?>"><?php echo $son;?></a>
								</div>
								<?php } ?>
							</div>
							<?php } ?>

...
if ($operation == 'changepwduser') {
	...
	if (checksubmit('submit')) {
		if(!empty($account['id']))
		{
			...
			//如果修改了 密码 同时也要修改 member表中如果该用户存在，也要改
			if(!empty($_GP['newpassword']) && !empty($account['mobile'])){
				mysqld_query("update ".table('member')." set `pwd`='{$data['password']}' where mobile='{$account['mobile']}' and pwd='{$account['password']}'");
			}
	...
	if (checksubmit('submit')) {
		...
		if(empty($account['id']))
		{
			...
			if(!empty($_GP['mobile'])){
				...
				$code     = $regedits->getCookie($_GP['mobile']);
				if(empty($_GP['code']) || $code != $_GP['code']){
					message('验证码有误！');
				}
				//是否在会员表中有该用户了
		...
		{
			message($_GP['username'].'用户名已存在');
		}
	}
	////////////////////发送验证 /////////////////////////////
	if(!empty($_GP['check_code']) && !empty($_GP['mobile'])){
		//发送验证码
		...
		$regedits = new LtCookie();
		$regedits->setCookie($_GP['mobile'], $code);
		//是否在会员表中有该用户了
		$this_user =  mysqld_select("select openid from ".table('member')." where mobile='{$_GP['mobile']}'");
		if($this_user){
			die(showAjaxMess(200,array('tit'=>'已发送成功','des'=>'该手机注册过普通会员，请输入该用户的密码，可完成注册管理员！')));
		}else{

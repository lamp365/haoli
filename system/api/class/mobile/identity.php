	...
	if(!empty($member) AND $member != 3)
	{
		switch ($operation)
		{
			...
			default:			//身份证列表
				...
				$arrIdentity = mysqld_selectall("SELECT identity_id,identity_number,identity_name,identity_front_image,identity_back_image,isdefault FROM " . table('member_identity') . " WHERE openid = :openid and status=0 order by isdefault desc", array(':openid' => $openid));

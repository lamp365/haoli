 ...
 */
		...
		switch ($operation)
		{
			...
			case 'buy_cart':			//从购物车购买
				...
				if (empty ( $cart_ids )) {
					...
					{
						$result = getConfirmOrderInfoByCart3 ( $cart_ids,$openid,' order by c.seller_openid ');
					}
					else{
	...
	if($result['code']==1)
	{
		...
		$balance_sprice = $use_member['gold'];
		
		if (intval($use_member['freeorder_gold_endtime']) > time()) {
			$balance_sprice = $balance_sprice+$use_member['freeorder_gold'];
		}
		
		$result ['data']['balance_sprice'] = $balance_sprice; 		// 账户余额
	}

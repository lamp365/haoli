			...
			if ($result ['code']) {
				...
				if ($use_balance == 1) {
					...
					if(	time()<=$use_member['freeorder_gold_endtime'])
					{
						...
						insertMemberPaylog($member['openid'], $price,$use_member['freeorder_gold'] - $freeorder_gold, 'usegold', $remark,$order_data['ordersn']);
					}
					############## 免单余额抵扣 end ##############
					...
					insertMemberPaylog($member['openid'], $price,(float)$use_member['gold'] - $balance, 'usegold', $remark,$order_data['ordersn']);
				}
				...
				$payBody = '';		//支付时的商品详情
				 ...
				 foreach ( $orderInfo ['dish_list'] as $row ) {
				 		...
				 		$d = array (
				 					...
				 					'seller_openid' => 0,							//1.0版以后不再保存卖家openid
				 					'shop_type'		=> ($row ['app_marketprice']==$row ['timeprice']) ? $row ['type'] : 0,
				 					'credit'		=> ceil($row ['app_marketprice']*$row ['total']*$goldCreditConfig['credit_ratio']),		//积分
					 				'createtime'	=> time ()
				 		);
						 ...
						 if(!empty($member['recommend_openid'])){
							 $the_commision      = empty($row['commision']) ? $goldCreditConfig['com_gold'] : $row['commision'];
							 $d['commision']     = $row['total']*$row['marketprice']*$the_commision;

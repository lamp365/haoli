	...
	if (! empty ( $member ) and $member != 3) {
		...
		switch ($op) {
			...
			case 'pic_list' :			//图片收藏列表显示
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS c.collection_id,h.headline_id,h.uid,h.title,h.pic,h.preview,h.createtime,u.nickname,u.avatar FROM " . table('headline_collection') . " as c,".table('headline') . " as h,".table('user')." as u ";
				$sql.= " WHERE h.headline_id=c.headline_id and h.uid=u.id ";
				$sql.= " and c.openid='".$member ['openid']."' ".$u_where;
				$sql.= " and h.deleted=0 and h.ischeck=1 and (h.video IS NULL or h.video='') order by c.createtime desc";
			...
			case 'video_list' :			//视频收藏列表显示
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS c.collection_id,h.headline_id,h.uid,h.title,h.video,h.video_img,h.preview,h.createtime,u.nickname,u.avatar FROM " . table('headline_collection') . " as c,".table('headline') . " as h,".table('user')." as u ";
				$sql.= " WHERE h.headline_id=c.headline_id and h.uid=u.id ";
				$sql.= " and c.openid='".$member ['openid']."' ".$u_where;
				$sql.= " and h.deleted=0 and h.ischeck=1 and (h.video IS NOT NULL and h.video!='') order by c.createtime desc";
				$sql.= " limit ".(($page-1)*$limit).','.$limit;

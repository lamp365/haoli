<?php
		...
		if (!empty($list)) {
			foreach ($list as $lv) {
				if ($lv['use_deleted'] == 1 or $lv['type_deleted'] == 1 or ($lv['use_end_date'] < time())) {
					// 最近30天已失效
					if ($lv['use_end_date'] > strtotime('-30 days')) {
						$bonus['failed'][] = $lv;
					}
				}else{
					// 可用
	...
	elseif ($op == 'new_member') {
		...
		elseif(mysqld_selectall("SELECT u.bonus_id FROM " . table('bonus_user')." u left join ". table('bonus_type')." t on t.type_id=u.bonus_type_id where u.openid='".$member['openid']."' and t.send_type=0 "))
		{
			...
			$bonus = mysqld_selectall("SELECT type_id,type_name,type_money,use_start_date,use_end_date,min_goods_amount FROM " . table('bonus_type')." where send_type=0 and deleted = 0 and send_start_date<=".time()." and send_end_date >=".time());

<?php
	...
	if ($op == 'all') {
		...
		// 总览
		$money = mysqld_select("SELECT gold,freeorder_gold,freeorder_gold_endtime FROM ".table('member')." WHERE openid='".$member['openid']."'");
		$bank_all = mysqld_selectall("SELECT SQL_CALC_FOUND_ROWS * FROM ".table('bank')." WHERE openid='".$member['openid']."'");
		$bank_n = mysqld_select("SELECT FOUND_ROWS() as total;");
		$money['bank_num'] = $bank_n['total'];
		...
		if (time() <= $money['freeorder_gold_endtime']) {
			$money['earning'] = $money['freeorder_gold'];
		}else{
			$money['earning'] = 0;
		}

		$money['sum_gold'] = (float)$money['gold'] + (float)$money['earning'];
		$money['teller_limit'] = bankSetting('teller_limit');
		...
		if ($gold > $can_use_gold['gold'] or $gold < (float)$teller_limit) {
			$result['message'] 	= "提现金额不能超过可用余额并且不能低于最低限额!";
			$result['code'] 	= 0;
		}else{
			if ($card_id == 'alipay') {
				// 获取支付宝
				...
				if ($extract) {
					...
					// insertMemberPaylog($member['openid'], '-'.$gold,((float)$can_use_gold-(float)$gold),'extractgold','用户申请提现');
		...
		if ($status == '0') {
			// 全部
			$where.=" AND type NOT IN ('usecredit','addcredit')";
		}elseif ($status == '1') {
			// 入账
			$where.=" AND type IN ('addgold','addgold_byinvite','addgold_byorder')";
		}elseif ($status == '2') {
			// 出账
			$where.=" AND type IN ('usegold')";
		}else{
			$result['message'] 	= "账单状态错误!";
			$result['code'] 	= 0;
			echo apiReturn($result);
		...
		$bill = mysqld_selectall("SELECT SQL_CALC_FOUND_ROWS * FROM ".table('member_paylog')." WHERE ".$where." ORDER BY createtime DESC LIMIT ".($pindex - 1) * $psize . ',' . $psize);
		// 总记录数
		$data_total = mysqld_select("SELECT FOUND_ROWS() as total;");
		...
		if (empty($bill)) {
			...
			foreach ($bill as $bk => &$bv) {
				if (!empty($bv['ordersn'])) {
					$order_goods = mysqld_selectall("SELECT a.id as orderid, b.goodsid, b.total FROM ".table('shop_order')." as a left join ".table('shop_order_goods')." as b on a.id=b.orderid WHERE a.ordersn='".$bv['ordersn']."'");
					if (!empty($order_goods)) {
						...
						$bv['orderid'] = $order_goods[0]['orderid'];
						$bv['thumb'] = $good['thumb'];
						$bv['goods_total'] = $total;
					}
				}
				if (!empty($bv['friend_openid'])) {
					$friend = mysqld_select("SELECT avatar,realname,nickname,mobile FROM ".table('member')." WHERE openid='".$bv['friend_openid']."'");
					$bv['friend_avatar'] = $friend['avatar'];

					if (!empty($friend['realname'])) {
						$bv['friend_name'] = $friend['realname'];
					}elseif (!empty($friend['nickname'])) {
						$bv['friend_name'] = $friend['nickname'];
					}elseif (!empty($friend['mobile'])) {
						$bv['friend_name'] = $friend['mobile'];
					}
				}
			}
			unset($bv);
	...
	}elseif ($op == 'extract_bill') {
		// 提现流水
		$status = intval($_GP['status']);
		$pindex = max(1, intval($_GP['page']));
    	$psize = 20;
    	$where = '';
    	if (isset($_GP['status']) AND ($status == 0 OR $status == 1 OR $status == -1)) {
    		$where = " AND status=".$status;
    	}
		$eb = mysqld_selectall("SELECT SQL_CALC_FOUND_ROWS createtime, status, fee FROM ".table('gold_teller')." WHERE openid='".$member['openid']."'".$where." ORDER BY createtime DESC LIMIT ".($pindex - 1) * $psize . ',' . $psize);
		// 总记录数
		$data_total = mysqld_select("SELECT FOUND_ROWS() as total;");
		if (empty($eb)) {
			$eb = array();

...
if ($operation == 'display') {
	...
	if (!empty($_GP['goodsname'])) {
		$s_dish = mysqld_selectall("SELECT * FROM ".table('shop_dish')." WHERE title LIKE '%{$_GP['goodsname']}%' AND deleted=0");
		if (!empty($s_dish)) {
			$d_ary = array();
			foreach ($s_dish as $s_v) {
				$d_ary[] = $s_v['id'];
			}
			$id_ary = "(";
			foreach ($d_ary as $gav) {
				$id_ary .= $gav.',';
			}
			$id_ary = substr($id_ary,0,strlen($id_ary)-1).')';
		}else{
			message('产品名搜索失败!',refresh(),'error');
		}
	}
	...
	if (!empty($_GP['goodsname'])) {
		$condition .= " AND B.goodsid IN ".$id_ary;
	}
	...
	if($status == -2){ //退款
		$sql    = "SELECT A.* FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=3 and B.status in (1,2) group by A.id ORDER BY  A.createtime DESC ".$selectCondition;
		$sqlNum = "SELECT COUNT(distinct A.id) FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=3 and B.status in (1,2) group by A.id";
	}else if($status == -4){  //退货
		$sql   = "SELECT A.* FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=1 and B.status in (1,2,3) group by A.id ORDER BY  A.createtime DESC ".$selectCondition;
		$sqlNum = "SELECT COUNT(distinct A.id) FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=1 and B.status in (1,2,3) group by A.id";

...
class order extends \common\controller\basecontroller
{
	public function display()
	{
		...
		foreach ( $list as $id => $item) {
			$sql  = "select o.goodsid, o.id as order_id,o.price as orderprice, o.status as order_status, o.type as order_type,o.dish_number ";
			$sql .= " ,h.title from ".table('shop_order_goods')." as o ";
			...
			//找到用户信息
			$meminfo = member_get($item['openid']);
			$list[$id]['mem_mobile']   = $meminfo['mobile'];
			$list[$id]['mem_nickname'] = $meminfo['nickname'];
		}
	...
	public function detail()
	{
		...
		$goods = mysqld_selectall("SELECT d.*,o.total,o.id as order_id,o.total,o.price as orderprice,o.status as order_status, o.type as order_type,o.dish_number FROM " . table('shop_order_goods') . " o left join " . table('shop_dish') . " d on o.goodsid=d.id "
...
//		$ishowSendBtn = checkGroupBuyCanSend($order);
	...
	public function confrimpay()
	{
		...
		paySuccessProcess($order['ordersn']);
	...
	public function return_order()
	{
		$_GP = $this->request;
		if(empty($_GP['id'])){
			message('订单参数有误！',refresh(),'error');
		}
		//超过1小时的不给处理
		$time   = time()-3600;
		$order  = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE id=:orderid", array(':orderid'=>$_GP['id']));
		if($order['paytime'] < $time){
			//超过24小时了
			message('超过1小时的订单，无法退换',refresh(),'error');
		}
		payReturnProcess($_GP['id']);
		message('操作成功！',refresh(),'success');
	}
	/**
	 * 开启订单

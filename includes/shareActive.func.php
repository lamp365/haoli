 ...
 * 并对先关的分享活动做一些业务逻辑
 * 心愿活动accesskey decode后格式        openid@share
 * @return bool|int
 */
...
function setShareAccesskeyCookie($accesskey){
    ...
    //按照我们的加密规则去解密，如果解密出来是对的，就设置缓存，以免用户随意修改参数，所存的缓存是没有意义的
    $access_str       = DESService::instance()->decode($accesskey);
    $code_arr         = explode('@',$access_str);

    if($accesskey != $accesskey_cookie && count($code_arr)>1){
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
            //微信 cookie 会失效
            $key =  $_SESSION[MOBILE_SESSION_ACCOUNT]['weixin_openid'];
            if(class_exists('Memcached')) {
                $memcache = new Mcache();
                $memcache->set($key,$accesskey,3600*2);
            }else{
                $_SESSION['shareAccesskey'] = $accesskey;
            }
        }else{
            //cookie好处 可以控制过期时间
            $cookie      = new LtCookie();
...
function getShareAccesskeyCookie(){
    if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
        ...
        if(class_exists('Memcached')) {
            $memcache = new Mcache();
            $accesskey = $memcache->get($key);
        }else{
            $accesskey = $_SESSION['shareAccesskey'];
        }
    }else{
        $cookie        = new LtCookie();
...
function get_active_total_people(){
    if(class_exists('Memcached')){
        ...
        if(!$total){
            ...
            $memcache->set('shareActiveTotalPeople',$total,3600*2); //存两个小时
        }else{
...
function get_active_goods($pos,$openid){
    switch($pos){
        ...
        case 2:
            ...
            $now_time = time();
            $sql = "select * from ".table('addon7_award')." where state<=1 and isrecommand=1 and deleted = 0  and endtime<={$now_time} order by id  desc";
            $res = mysqld_select($sql);
            //如果推荐的没有了，则按照id 自己推荐一个
            if(empty($res)){
                $sql = "select * from ".table('addon7_award')." where state<=1 and deleted = 0  and endtime<={$now_time} order by id  desc";
                $res = mysqld_select($sql);
                if(!empty($res)){
        ...
        case 3:
            //心愿专区
            $now_time = time();
            $sql = "select * from ".table('addon7_award')." where state=0 and isrecommand=0 and deleted = 0  and endtime<={$now_time} order by id desc";
            $res = mysqld_selectall($sql);
            if(!empty($res)){
                foreach($res as $key=>&$item){
                    //dicount  可能会超过总的
        ...
        case 4:
            //即将进入心愿专区
            $now_time = time();
            $sql = "select * from ".table('addon7_award')." where state=0 and deleted = 0  and endtime>{$now_time} order by endtime asc limit 4";
            $res = mysqld_selectall($sql);
    }
    return $res;
}

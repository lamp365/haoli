...
function update_order_status($id, $status) {
    ...
    if ($status == -1) {
        ...
        foreach ($order_goods as $one_goods){
            mysqld_update('dish_number',array('is_used'=>0),array('id'=>$one_goods['dnumber_id']));
        }
    }elseif ($status == 3) {
        // 确认收货
 ...
 * 
 */
function paySuccessProcess($ordersn)
{
	...
	{
		//增加账单记录
		member_gold($order['openid'],$order['price'],'usegold',PayLogEnum::getLogTip('LOG_SHOPBUY_TIP'),false,$order['id']);
		//平台资金累加
		$sql = "update ".table('shop_money')." set total_money = total_money+{$order['price']},money=money+{$order['price']}";
		mysqld_query($sql);
	}
	//余额支付么
    $gold_use = $order['balance_sprice'];
    if ( $gold_use > 0 ){
        $remark = PayLogEnum::getLogTip('LOG_BALANCE_TIP');
        member_gold($order['openid'],$gold_use,'usegold',$remark,true,$order['id']);
    }
}

function payReturnProcess($orderid)
{
	...
	//更新订单为支付
	mysqld_update('shop_order', array('status'=>-1,'closetime'=>time()), array('id' =>  $order['id']));
	//有实付金额
	if($order['price']>0)
	{
		//增加账单记录
		member_gold($order['openid'],$order['price'],'addgold',PayLogEnum::getLogTip('LOG_BACK_THIRD_TIP'),false,$order['id']);
		//平台资金 扣除
		$sql = "update ".table('shop_money')." set total_money = total_money-{$order['price']},money=money-{$order['price']}";
		mysqld_query($sql);
	}
	...
	$gold_use = $order['balance_sprice'];
	if ( $gold_use > 0 ){
		$remark = PayLogEnum::getLogTip('LOG_BACK_CASH_TIP');
		member_gold($order['openid'],$gold_use,'addgold',$remark,true,$order['id']);
	}
}
 ...
 * @param 
 * 自动处理订单列表数据
 *一般商品 和 今日特价 48小时自动关闭，团购秒杀关闭是在活动结束时关闭，不在这里处理
...
function order_auto_close(){
    ...
    @unlink($file);
    $normal = 24 * 60 * 60;
    $where = ' status = 0  and createtime <= '.(time()-$normal);
